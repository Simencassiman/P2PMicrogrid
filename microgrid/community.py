    ...
    if is_testing:
        for a in range(len(community.agents)):
            for r in range(rounds + 1):
                db.log_rounds_decision(con, setting, a, days, time, r, community.decisions[:, r, a].tolist())
...
def load_and_run(con: Optional[sqlite3.Connection] = None, is_testing: bool = False, analyse: bool = True) -> None:
    ...
    for i in range(nr_pv_drop):
        agent_dfs[i].loc[env_df['time'] > 0.5, 'pv'] = 0
    if homogeneous:
    ...
    for day in days:
        ...
        if con:
            print("Saving...")
            save_community_results(con, is_testing, pv_setting, int(day), community, cost.numpy())
...
nr_pv_drop = 1
pv_setting = f'{nr_agents}-agent-{nr_pv_drop}-pv-drop-com'
